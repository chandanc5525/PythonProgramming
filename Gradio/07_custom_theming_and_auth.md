...
- ⚠️ Just like Streamlit's `st.secrets` (Streamlit course, Module 08), **never hardcode real passwords directly in source code** committed to Git — load them from an environment variable instead: `auth=("admin", os.environ["PYRAIL_ADMIN_PW"])`.
